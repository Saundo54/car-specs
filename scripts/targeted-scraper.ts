...
async function main() {
  const targetMakes = ['mazda', 'toyota', 'kia', 'hyundai', 'mitsubishi', 'nissan'];
  console.log(`Starting FORCE scraper for: ${targetMakes.join(', ')} (${START_YEAR}-${END_YEAR})...`);
  ...
  if (await fs.pathExists(VEHICLES_FILE)) {
    vehicles = await fs.readJson(VEHICLES_FILE);
    console.log(`Loaded ${vehicles.length} existing vehicles.`);
  }
          ...
          try {
            ...
            if (variantUrls.length > 0) {
              ...
              const scrapePromises = variantUrls.map(url => limit(async () => {
                ...
                // FORCE SCRAPE: If we already have this ID, we check if we have full specs.
                // If the existing entry has very few mechanical specs, we re-scrape.
                const existing = vehiclesMap.get(id);
                if (existing && existing.specs?.mechanical && Object.keys(existing.specs.mechanical).length > 5) {
